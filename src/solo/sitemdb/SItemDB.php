...
class SItemDB extends PluginBase{

  public static $prefix = "§l§b[SItemDB] §r§7";
  ...
  private $db_name = []; // KEY : simplized item name  VALUE : ItemInfo
  private $db_item = []; // KEY : serialized item      VALUE : ItemInfo
  ...
  public function onEnable(){
    ...
    foreach([
      ...
      "ItemRegisterCommand",
      "ItemSearchCommand",
      "ItemUnregisterCommand",
      "ItemViewCommand"
    ] as $class){
      $class = "\\solo\\sitemdb\\command\\" . $class;
      $this->getServer()->getCommandMap()->register("stiemdb", new $class($this));
    }
  }
  ...
  public function registerItem(Item $item, string $customName, string $description = "", bool $force = false) : bool{
    if(($info = $this->getItemInfo($customName)) !== null || ($info = $this->getItemInfoByItem($item)) !== null){
      if($force){
        $this->unregisterItemInfo($info);
        $this->getServer()->getLogger()->info(SItemDB::$prefix . "아이템 " . $customName . "이 기존 \"" . $info->getName() . "\" 아이템에 덮어쓰여 기록되었습니다.");
      }else{
        $this->getServer()->getLogger()->critical(SItemDB::$prefix . "아이템 " . $customName . " 등록에 실패하였습니다 : 이미 존재하는 아이템입니다 (" . $info->getName() . ")");
        return false;
      }
    }
    $this->registerItemInfo(new ItemInfo($item, $customName, $description));
    return true;
  }
  ...
  public function parseItem(string $input){
  ...
  public function searchItem(string $keyword){
    ...
    foreach($this->db_name as $name => $info){
      if(strpos($name, $keyword) !== false){
        $result[$info->getName()] = clone $info->getItem();
      }
    }
    return $result;
  }

  public function unregisterItem(string $name){
    if(($info = $this->getItemInfo($name)) !== null){
      $this->unregisterItemInfo($info);
      return true;
    }
  ...
  public function registerItemInfo(ItemInfo $info){
    $this->db_name[$this->clean($info->getName())] = $info;
    $this->db_item[json_encode($info->getItem())] = $info;
  }

  public function getItemInfo(string $name){
    return $this->db_name[$this->clean($name)] ?? null;
  }

  public function getItemInfoByItem(Item $item){
    $item = clone $item;
    $item->setCount(1);
    return $this->db_item[json_encode($item)] ?? null;
  }

  public function getAllItemInfo() : array{
    return $this->db_name;
  }

  public function unregisterItemInfo(ItemInfo $info){
    unset($this->db_name[$this->clean($info->getName())]);
    unset($this->db_item[json_encode($info->getItem())]);
  }

  private function clean(string $name){
    return trim(str_replace(['-', '_', '.', ' '], ['', '', '', ''], TextFormat::clean(strtolower($name))));
  }

  public function save(){
    $array = [];
    foreach(self::getAllItemInfo() as $info){
      $array[] = $info;
    }
    file_put_contents($this->getDataFolder() . "itemdb.json", json_encode($array));
  }

  private function load(){
    if(file_exists($this->getDataFolder() . "itemdb.json")){
      $array = json_decode(file_get_contents($this->getDataFolder() . "itemdb.json"), true);
      foreach($array as $data){
        $info = ItemInfo::jsonDeserialize($data);
        self::registerItemInfo($info);
      }
    }else{ // init
      $this->updateDefaultItemDB();
    }
  }

  private function updateDefaultItemDB(){
    $defaults = [
      "0:0" => "공기",
  		"1:0" => "돌",
  		"1:1" => "화강암",
    ...
    foreach($defaults as $id => $name){
      if(self::getItem($name) === null){
        self::registerItem(Item::get(...array_map("intval", explode(":", $id))), $name);
      }
    }
  }
}
